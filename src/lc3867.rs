...
impl Solution {
    ...
    // fn main() {
    //     let result = Solution::gcd_sum( vec![3,6,2,8]);
    //     println!("{}", result);
    // }
    ...
    pub fn gcd_sum(nums: Vec<i32>) -> i64 {
        ...
        let mut v = Self::get_prefix_gcd(nums);
        v.sort();
        let mut sum:i64 = 0;
        let len = v.len();
        for i in 0..len/2{
            sum+=Self::gcd(v[i],v[len-i-1]) as i64;
        }
    ...
    fn get_prefix_gcd(nums: Vec<i32>) -> Vec<i32>{
        let mut max = 0;
